...
```
./tools/release/windows/build.sh --with-cli
```
...
`tools/parseh-tts/speak.sh` to read the prompt aloud (Persian default).
...
Shells out to `tools/parseh-tts/speak.sh` (which auto-detects
piper-tts or espeak-ng locally). Exits with code 3 if the wrapper is
not present.

### `parseh stt [--seconds N]`

Shells out to `tools/parseh-stt/listen.sh` (V0.3+ — wrapper does not
...
- **`parseh-tts`** — `parseh tts` shells out to the existing wrapper at
  `tools/parseh-tts/speak.sh`. Privacy: the wrapper uses local
  piper/espeak-ng; no network egress.
- **`parseh-stt`** — `parseh stt` looks for
  `tools/parseh-stt/listen.sh` and falls back gracefully when missing.
