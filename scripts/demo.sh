...
echo
echo "==> [1/3] Building core binaries (parseh-miner, parseh CLI) — release"
...
echo
echo "==> [2/3] Generating a fresh ed25519 node identity (sandboxed in \$TMPDIR)"
"$MINER" --config-dir "$WORK" init
echo "    --- whoami ---"
"$MINER" --config-dir "$WORK" whoami

echo
echo "==> [3/3] 3-node acceptance test (deterministic in-process replay)"
